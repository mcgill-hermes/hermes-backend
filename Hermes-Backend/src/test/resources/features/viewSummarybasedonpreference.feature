Feature: View news summary based on categorical preference
  ...
  Scenario Outline: View all the suggested articles based on the current user preference
    ...
    And the system exist the account "<username>", "<password>", "<firstname>", "<lastname>"
    ...
    Examples:
      | username | password | firstname | lastname| category  | newsID  | number
      | user01   | code01   | first01   | last01  | category1 | news1   | 1
      | user02   | code02   | first02   | last02  | category2 | news2   | 2
  ...
  Scenario Outline: View all suggested articles based on the current user but the user has no preference (alternative case)
    ...
    Examples:
      | username | password | firstname | lastname| category  | newsID  | number
      | user01   | code01   | first01   | last01  | category1 | news1   | 0
